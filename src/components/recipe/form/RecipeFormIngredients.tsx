...
function SortableIngredient({
  ...
  return (
    <div ref={setNodeRef} style={style} className="flex items-start gap-2">
      ...
      <Input placeholder={t('form.quantity')} className="w-24" {...register(`ingredients.${index}.quantity`)} />
      <Input placeholder={t('form.unit')} className="w-20" {...register(`ingredients.${index}.unit`)} />
      <Input placeholder={t('form.ingredient')} className="flex-1" {...register(`ingredients.${index}.name`)} />
      {canRemove && (
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle>{t('form.ingredients')}</CardTitle>
